...
class TestRunPipelineSuccess:
    ...
    def test_full_pipeline_execution_order(
        ...
        # Verify call order
        mock_setup.assert_called_once_with(settings.database_url)
        mock_get_conn.assert_called_once_with(settings.database_url)
        mock_extract.assert_called_once_with(mock_conn)
        mock_create.assert_called_once_with(
            [c.content for c in chunks],
            settings,
        )
        mock_store.assert_called_once_with(mock_conn, chunks, embeddings)
    ...
    def test_uses_get_settings_when_none_provided(
        ...
        with patch("src.embeddings.cli.get_settings") as mock_get_settings:
            ...
            mock_setup.assert_called_once_with(fake_settings.database_url)
...
class TestRunPipelineErrors:
    ...
    @patch("src.embeddings.cli.get_connection")
    @patch("src.embeddings.cli.setup_tables")
    def test_get_connection_failure_exits(
    ...
    @patch("src.embeddings.cli.get_connection")
    @patch("src.embeddings.cli.setup_tables")
    def test_conn_closed_even_on_error(
